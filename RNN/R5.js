...
async function run() {
  ...
  // Save the model (no check for file size after save)
  //await model.save('downloads://CIFAR_1_64_model');
...
function getModel() {
  ...
  // Reshape input image (32x32 with 3 channels) into a sequence.
  model.add(tf.layers.reshape({
    targetShape: [32, 96],  // 32 timesteps of 96 features (32*3)
    inputShape: [32, 32, 3]
  }));

  // First RNN layer with returnSequences set to true.
  model.add(tf.layers.simpleRNN({
    units: 1024,
    activation: 'relu',
    returnSequences: true,  // <-- This makes the layer output a 3D tensor.
